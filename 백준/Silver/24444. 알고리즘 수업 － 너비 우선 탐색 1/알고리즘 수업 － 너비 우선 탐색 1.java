...
public class Main {
    ...
    public static void bfs(int node) {
        ...
        queue.add(node);      // 시작 정점을 큐에 추가
        visitOrder[node] = order++; // 시작 정점의 방문 순서 기록

        while (!queue.isEmpty()) { // 큐가 비어 있지 않을 동안 반복
            int curr = queue.remove(); // 큐에서 정점을 꺼냄

            // 현재 정점과 연결된 모든 정점을 확인
            for (int next : Graph.get(curr)) { // 연결된 정점 탐색
                if (!visited[next]) { // 방문하지 않았다면
                    visited[next] = true;      // 방문 처리
                    queue.add(next);           // 큐에 추가
                    visitOrder[next] = order++; // 방문 순서 기록
                }
            }
        }
    }
